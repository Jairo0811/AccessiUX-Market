...
{
    options.AddPolicy("Frontend", policy => policy.WithOrigins(allowedOrigins).AllowAnyHeader().AllowAnyMethod().AllowCredentials());
});
...
app.UseStatusCodePages(async statusCodeContext =>
{
    ...
    await Results.Problem(statusCode: response.StatusCode, title: response.StatusCode switch
    {
...
app.MapGet("/api", () => Results.Ok(new { name = "AccessiUX Market API", version = "0.3.0" }));
