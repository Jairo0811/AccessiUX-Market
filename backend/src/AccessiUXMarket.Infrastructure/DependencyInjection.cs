        ...
        IConfiguration configuration)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            throw new InvalidOperationException(
                "ConnectionStrings:DefaultConnection must be configured at runtime.");
        }
        ...
        services.AddOptions<PasswordResetOptions>()
            ...
            .Validate(
                options => Uri.TryCreate(options.FrontendUrl, UriKind.Absolute, out _),
                "PasswordReset:FrontendUrl must be an absolute URL.")
            .ValidateOnStart();
